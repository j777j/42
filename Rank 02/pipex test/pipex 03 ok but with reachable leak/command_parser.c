...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/06 15:48:22 by juwang            #+#    #+#             */
/*   Updated: 2025/07/06 15:56:00 by juwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		size_t i)
{
	...
	if (!cmd_args[i])
	{
		...
		handle_system_error("Memory alloc failed for cmd arg");
		return (0);
	}
	memcpy(cmd_args[i], current_pos, len);
	cmd_args[i][len] = '\0';
	return (1);
}
...
char	**parse_command_string(const char *cmd_str)
{
	...
	cmd_args = handle_initial_cmd_alloc(cmd_str, &num_args);
	if (!cmd_args)
		return (NULL);
	i = 0;
