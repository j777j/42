...
void	load_sprites(t_game *game)
{
	...
	game->enemy[0].ptr = mlx_xpm_file_to_image(game->mlx, "textures/enemy_1.xpm", &w, &h);
	game->enemy[1].ptr = mlx_xpm_file_to_image(game->mlx, "textures/enemy_2.xpm", &w, &h);
	...
	if (!game->wall.ptr || !game->floor.ptr || !game->collectible.ptr
		...
		|| !game->enemy[0].ptr || !game->enemy[1].ptr) // <- 新增敵人檢查
	{
	...
	game->enemy[0].addr = mlx_get_data_addr(game->enemy[0].ptr,
			&game->enemy[0].bpp, &game->enemy[0].line_len, &game->enemy[0].endian);
	game->enemy[1].addr = mlx_get_data_addr(game->enemy[1].ptr,
			&game->enemy[1].bpp, &game->enemy[1].line_len, &game->enemy[1].endian);
