...
static void	handle_dollar_sign(char **word_ptr, t_expand_state *state, t_shell *shell)
{
	...
	if (var_value)
	{
		if (!state->in_dquotes)
		{
			split_value = ft_split(var_value, ' ');
			if (split_value && split_value[0])
			{
				append_str_to_word(&state->current_word, split_value[0]);
				finalize_word(state);
				j = 1;
				while (split_value[j])
				{
					append_str_to_word(&state->current_word, split_value[j]);
					if (split_value[j + 1])
						finalize_word(state);
					j++;
				}
			}
			ft_split_free(split_value);
		}
		else
			append_str_to_word(&state->current_word, var_value);
	}
