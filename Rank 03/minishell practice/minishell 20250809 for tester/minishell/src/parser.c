...
static t_cmd	*parse_seq(t_list **tokens, t_shell *shell);
...
static t_cmd	*parse_seq(t_list **tokens, t_shell *shell)
{
	...
	if (peek_type(tokens) == TOKEN_SEMICOLON)
	{
		...
		if (peek_type(tokens) == TOKEN_EOF || peek_type(tokens) == TOKEN_PIPE || peek_type(tokens) == TOKEN_SEMICOLON)
			return (parser_error("Syntax error near unexpected token `;'", shell, cmd));
		cmd = new_seq_cmd(cmd, parse_seq(tokens, shell));
	}
	return (cmd);
}
