...
/*   Updated: 2025/08/13 19:30:00 by juwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_list	*expand_and_split_arg(char *arg, t_shell *shell)
{
	t_split_state	state;
	int				i;
	t_expand_result	exp_result;

	state.is_assignment = ft_strchr(arg, '=') != NULL;
	exp_result = perform_expansion(arg, shell);
	state.expanded_str = exp_result.str;
	if (!state.expanded_str)
		return (NULL);
	if (exp_result.was_quoted || state.is_assignment)
	{
